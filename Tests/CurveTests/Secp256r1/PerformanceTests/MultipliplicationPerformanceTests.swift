//
//  MultipliplicationPerformanceTests.swift
//  SwiftCryptoTests
//
...
class MultipliplicationPerformanceTests: XCTestCase {
    ...
    // Using `measure` closure: Takes real time ~1 minute, measured time ~4 seconds
    func test30HighMultiplications() {
        ...
        func toNumber(_ hex: String) -> Number {
            return Number(hexString: hex)!
        }
        ...
        self.measure {
            for D in 1...count {
                let p = Secp256r1.G * (Secp256r1.P - Number(D))
                calculated.append(p)
            }
        }
    ...
    func test1000LowMultiplications() {
        ...
        let twoCharsFromCalculated = calculated.map { lastTwoCharsOfX($0) }
