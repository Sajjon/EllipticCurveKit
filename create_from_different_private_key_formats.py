...
def debug_pk(priv):
    debug(priv)    
    hexlified = binascii.hexlify(priv)
    print "Hexlified:"  
    debug(hexlified)    
    print "Length `priv`: %d" % len(priv)
    print "Length `hexlified`: %d" % len(hexlified)
...
class EllipticCurvePoint:
    ...
    def SignMessage(self,message,priv):
        ...
        (d,uncompressed)=self.DFromPrivateKeyWifBase58Encoded(priv)
    ...
    # def AddressFromPrivate(self,priv,is_sk_wif):
    #     #Transform a private key to a bitcoin address.
    #     if is_sk_wif:
    #         (d,uncompressed)=self.DFromPrivateKeyWifBase58Encoded(priv)
    #     else:
    #         (d,uncompressed)=self.DFromPrivateKey(priv)
            
    #     return self.AddressFromD(d,uncompressed)
    def AddressFromPrivateWif(self,priv):
        #Transform a private key to a bitcoin address.
        (d,uncompressed)=self.DFromPrivateKeyWifBase58Encoded(priv)    
        return self.AddressFromD(d,uncompressed)

    def AddressFromUncompressedPrivateHex64(self,priv):
        #Transform a private key to a bitcoin address.
        d=self.DFromPrivateKeyHex64(priv)
        return self.AddressFromD(d,True)    
    ...
    def DFromPrivateKeyHex64(self,sk):
        ...
        return self.DFromPrivK(bytes_array)
...
def main():
    ...
    print "CYON TESTING"
    sk_wif="5J8kgEmHqTH9VYLd34DP6uGVmwbDXnQFQwDvZndVP4enBqz2GuM"
    sk_wif_cmpr = "KxdDnBkVJrzGUyKc45BeZ3hQ1Mx2JsPcceL3RiQ4GP7kSTX682Jj"
    sk="29ee955feda1a85f87ed4004958479706ba6c71fc99a67697a9a13d9d08c618e"
    adr = bitcoin.AddressFromPrivateWif(sk_wif)
    assert adr == "157k4yFLw92XzCYysoS64hif6tcGdDULm6"
    adr2 = bitcoin.AddressFromPrivateWif(sk_wif_cmpr)
    assert adr2 == "1Dhtb2eZb3wq9kyUoY9oJPZXJrtPjUgDBU"

    (d_wif_uncmpr, _) = bitcoin.DFromPrivateKeyWifBase58Encoded(sk_wif)
    (d_wif_cmpf, _) = bitcoin.DFromPrivateKeyWifBase58Encoded(sk_wif_cmpr)
    debug(d_wif_cmpf)
    debug(d_wif_uncmpr)
    assert d_wif_uncmpr == d_wif_cmpf

    wif_uncmpr_decoded = b58decode(sk_wif)
    wif_cmpr_decoded = b58decode(sk_wif_cmpr)
    # debug(wif_uncmpr_decoded)
    # debug(wif_cmpr_decoded)
    # print("Hexlified:")
    # debug(binascii.hexlify(wif_uncmpr_decoded))
    # debug(binascii.hexlify(wif_cmpr_decoded))
    # assert wif_uncmpr_decoded == wif_cmpr_decoded
    # print "YES match after base58decode"

    d_h64 = bitcoin.DFromPrivateKeyHex64(sk)
    debug(d_h64)

    assert d_h64 == d_wif_cmpf

    print "YES!!! :D YES!!! :D YES!!! :D YES!!! :D YES!!! :D YES!!! :D "

    adr3 = bitcoin.AddressFromUncompressedPrivateHex64(sk)
    print "CREATED ADDRESSSES"
    debug(adr)
    debug(adr2)
    debug(adr3)
    assert adr == adr3
